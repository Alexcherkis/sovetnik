...
const Navbar: React.FC = () => {
  ...
  return (
    <>
              ...
              <nav className="hidden lg:flex items-center space-x-8">
                {NAV_ITEMS.map((item) => (
                  <div key={item.path} className="relative group h-full py-2">
                    <Link 
                      to={item.path} 
                      className={`text-sm font-bold uppercase tracking-wider transition-all flex items-center relative ${
                        isActive(item.path) ? 'text-brand-gold' : textClass
                          ...
                          <div className="relative bg-white z-10">
                            {item.subItems.map(sub => (
                              <Link 
                                key={sub.path} 
                                to={sub.path}
                                className={`block px-6 py-3 text-sm transition-colors border-l-4 font-medium font-serif ${
                                  isActive(sub.path) 
      ...
      <div 
        className={`lg:hidden fixed inset-0 bg-white z-40 transition-transform duration-300 ease-in-out flex flex-col pt-[80px] ${
                          ...
                          <div className="py-2">
                            {item.subItems.map(sub => (
                              <Link 
                                key={sub.path} 
                                to={sub.path} 
                                className={`flex items-center py-3 pl-10 pr-6 text-base transition-colors border-l-2 ml-6 mb-1 ${
                                  isActive(sub.path) ? 'text-brand-red border-brand-red font-bold' : 'text-slate-600 border-transparent'
                                }`}
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-brand-950 text-white pt-16 pb-8 border-t border-brand-900">
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 md:gap-12 lg:gap-16 mb-12">
        ...
        <div className="border-t border-brand-900 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-slate-600 gap-4 text-center md:text-left">
          <p>© {currentYear} Экспертное бюро "Советникъ". Все права защищены. Лицензия ФЭСЭП РФ.</p>
