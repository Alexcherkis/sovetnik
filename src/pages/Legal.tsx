...
const LegalHeader: React.FC<{ title: string; subtitle: string; icon: any }> = ({ title, subtitle, icon: Icon }) => (
  <div className="bg-brand-950 text-white pt-24 pb-28 md:pt-36 md:pb-32 relative overflow-hidden">
    {/* Abstract Background Elements */}
    <div className="absolute top-0 right-0 w-96 h-96 bg-brand-red/10 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-50"></div>
    <div className="absolute bottom-0 left-0 w-64 h-64 bg-brand-gold/5 rounded-full blur-3xl transform -translate-x-1/2 translate-y-1/2 pointer-events-none opacity-50"></div>

    <div className="container mx-auto px-5 md:px-6 relative z-10">
      <div className="max-w-4xl mx-auto">
        <Link
          to="/"
          className="inline-flex items-center text-slate-400 hover:text-white mb-8 transition-colors text-xs md:text-sm font-bold uppercase tracking-wider group bg-white/5 py-2 px-3 rounded-lg backdrop-blur-sm self-start"
        >
          <ArrowLeft size={16} className="mr-2 group-hover:-translate-x-1 transition-transform" />
          На главную
        </Link>

        <div className="flex flex-col md:flex-row md:items-center mb-6 gap-4 md:gap-6">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-brand-800 rounded-xl flex items-center justify-center text-brand-gold shadow-lg border border-brand-700 shrink-0">
            <Icon className="w-6 h-6 md:w-8 md:h-8" />
          </div>
          <h1 className="text-2xl md:text-5xl lg:text-6xl font-serif font-bold text-white leading-[1.2]">
            {title}
          </h1>
        </div>

        <p className="text-slate-300 text-sm md:text-xl font-light max-w-2xl leading-relaxed md:ml-[5.5rem] opacity-90">
          {subtitle}
        </p>
      </div>
    </div>
...
const LegalContent: React.FC<{ children: React.ReactNode; date: string }> = ({ children, date }) => (
  <div className="container mx-auto px-4 md:px-6 pb-20">
    <div className="max-w-4xl mx-auto -mt-12 md:-mt-16 bg-white rounded-xl md:rounded-2xl shadow-xl border border-gray-100 relative z-20 p-5 md:p-12 lg:p-16">
      <div className="mb-6 md:mb-8 border-b border-gray-100 pb-4 md:pb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-2 text-xs md:text-sm text-slate-400 uppercase tracking-wider font-semibold">
        <span>Юридический документ</span>
        <span>Обновлено: {date}</span>
      </div>
      <div className="space-y-8 md:space-y-12 text-slate-700 leading-relaxed text-sm md:text-base">
        {children}
      </div>
    </div>
  </div>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <section className="scroll-mt-24">
    <h2 className="text-lg md:text-2xl font-serif font-bold text-brand-900 mb-3 md:mb-6 flex items-start md:items-center">
      <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-brand-red mr-3 mt-2 md:mt-0 shrink-0"></span>
      {title}
    </h2>
    <div className="text-sm md:text-lg font-light text-slate-600 space-y-3 md:space-y-4 pl-0 md:pl-5 border-l-0 md:border-l border-gray-100">
      {children}
    </div>
  </section>
);
...
const ListItem: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <li className="flex items-start mb-2 last:mb-0">
    <span className="mr-3 text-brand-gold mt-1 shrink-0">•</span>
    <span>{children}</span>
  </li>
);
      ...
      <LegalContent date="12 Октября 2023">
        ...
        <div className="mt-12 p-6 bg-brand-50 rounded-lg border border-brand-100 text-sm md:text-base text-slate-600">
          ...
          <p>
            ...
            <a href="mailto:expert-sovetnik-rf@ya.ru" className="text-brand-red font-bold hover:underline ml-1" onClick={() => typeof window.ym !== 'undefined' && window.ym(YANDEX_METRICA_ID, 'reachGoal', 'click_email')}>expert-sovetnik-rf@ya.ru</a>
          </p>
        </div>
      </LegalContent>
    </div>
